...
# PREVIOUS_VALUES_XYZ = {(row, col): 0 for row in range(1) for col in range(3)}
# PREVIOUS_VALUES_RPY = {(row, col): 0 for row in range(1) for col in range(3)}

PREVIOUS_VALUES = {
    'tab_memsPosition': {(row, col): 0 for row in range(37) for col in range(3)},
...
def triggers(gui):
    ...
    gui.table_mountPos_rpy.itemChanged.connect(
        lambda item, le=gui.table_mountPos_rpy : on_item_changed(item, le))
    gui.table_mountPos_xyz.itemChanged.connect(
        lambda item, le=gui.table_mountPos_xyz: on_item_changed(item, le))
    gui.tab_memsPosition.itemChanged.connect(
        lambda item, le=gui.tab_memsPosition: on_item_changed(item, le))
...
def move(table, row, col, value):
    ...
    if table.objectName() == 'tab_memsPosition':
        ...
        # Check segment within limits
        # get new segment position
        # new_pos = device.get_segment_range(segment, 0, piston, tip, tilt, True)
        ...
        # wait for the device to move

        new_pos = device.get_pos(axis)
...
def on_item_changed(item, table):
    '''    
    psudeocode:
        1. get the new value
    ...
    try:
        ...
        # new_pos = move(table, row, col, float_value)
        # move(device, table, row, col, float_value)
